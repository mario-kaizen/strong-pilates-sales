...
export default function RealCost({
  ...
  const adSpend = monthlyAdData.map(d => d.spend)
  const adLeads = monthlyAdData.map(d => d.leads)
  const adPurchases = monthlyAdData.map(d => d.purchases)

  const lostIntros = Math.round(introsSold * (1 - introConversionRate / 100))
  const lostAdSpend = lostIntros * costPerIntro
  ...
  const cpaConfig = useMemo<ChartConfiguration>(() => ({
    type: 'bar',
    data: {
      labels: [`Cost Per Lead\n($${operatingAdSpend.toLocaleString()} \u00F7 ${operatingLeads.toLocaleString()} leads)`, `Cost Per Intro Offer\n($${operatingAdSpend.toLocaleString()} \u00F7 ${introsSold} intros)`, `Cost Per Lost Intro\n(${Math.round(100 - introConversionRate)}% never convert = $${costPerIntro.toFixed(0)} burned)`],
      datasets: [{
  ...
  return (
    <SectionWrapper
      ...
      title={`$${totalAdSpend.toLocaleString()} in Ad Spend — What Did It Actually Buy?`}
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
        <StatCard label="Total Lifetime Spend" value={`$${(totalAdSpend).toLocaleString()}`} sub={`${months[0]} — ${months[months.length - 1]}`} />
        <StatCard label="Pre-Open Spend" value={`$${presaleAdSpend.toLocaleString()}`} sub="Presale + opening period" />
        <StatCard label="Operating CPL" value={`$${operatingCpl.toFixed(2)}`} sub={`${operatingLeads.toLocaleString()} real leads`} variant="warning" />
        <StatCard label="Cost Per Intro Offer" value={`$${costPerIntro.toFixed(2)}`} sub={`${introsSold} intros sold (operating period)`} variant="warning" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '24px' }}>
        <InsightCard number={`$${operatingCpl.toFixed(2)}`} title="Operating CPL (Post-Opening)">
          <p>Once the studio was open and lead tracking was active, the real CPL across all lead types (forms, pixel, custom events — excluding page views) is ${operatingCpl.toFixed(2)}. That&apos;s the fair number to benchmark against.</p>
        </InsightCard>
        <InsightCard number={`$${costPerIntro.toFixed(2)} per Intro`} title="The Real Cost of an Intro Offer">
          <p>Operating spend (${operatingAdSpend.toLocaleString()}) divided by {introsSold} intro offers sold = ${costPerIntro.toFixed(2)} per intro buyer. {Math.round(100 - introConversionRate)}% of those never become members — that&apos;s {lostIntros} lost intros at ${costPerIntro.toFixed(0)} each = <strong>${(lostAdSpend / 1000).toFixed(0)}K+ in ad spend that led nowhere</strong>.</p>
        </InsightCard>
      </div>
